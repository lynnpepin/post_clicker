raw_posts = [
  ...
  "say what you will about elon musk, he did the impossible: get people to use Mastodon",
  "they're eating the nintendogs. they're eating the nintencats. and some, i assume, are good nintenpeople",
  "relaxing after a long day with a movie that sounds cute and cottagecore, 'the silence of the lambs'",
  ...
  "paid $400 for an IQ test. explained all the plotholes in Avengers: Infinity War. is 'inconclusive' good or bad",
  "haunting and deeply beautiful whistling from a man in the grocery store. one of the most beautiful things i've ever heard. didn't compliment him because i don't want to encourage that kind of behavior",
  "(turning 25) i've only got another 10 years of trick or treating left in me",
  "time to destroy bit coin",
...
//"fucked elf dot tumblr dot com using all his holes to spread christmas cheer",
  ...
  "just three words for you: windows 12 is here",
  "platonic ideal homosexual",
  ...
  "the woke left has cancelled Dr Seuss for 'the pocket book of boners' one too many times",
  "driving is like a nintendo Gamecube game. 'STEEL PLATES IN ROAD' yeah 'NEW TRAFFIC PATTERN' i gotta buy this",
  "if you ask me about the reward program i will kill myself right here. hey do you have apple pay",
  "man who secretly wants to be a cuck. but everyone he brings to fuck his wife just wants to watch. that is my tragedy",
  ...
  "oh",
  "End Of Road For GOTMILF License Plate - 'Offensive' vanity tag yanked by Washington officials. This is the story of GOTMILF.",
  "the secret to comedy is that i was sexually abused 20 years ago. yeah, like a lot. no i don't recommend it",
  "i'm in pain every day. this is a cry for help",
  "COFFEE REVIEW: this fucked up roasted-seed tea pretends to be a bean when it's not even a legume. burnt, crushed to smithereens, and boiled away, it makes an astringent bitter brown brew that will leave you jittering and begging for more. 4/4, delicious",
  "lyndon really do be johnsin'",
  "santa claus is at this mall? what are the odds?",
  "if bitcoin were any good there would be butchcoin. think about it",
  "breathign a sigh of relief after Better Help told me hatsune miku isnt real. i could not f orgive her if she is real",
  "if a centrist were to rank beverages would they say 'sodas aren't best hot or warm, they're best room temperature' or would they say 'the most flavorful beverage is water'",
  "(planning a police kink scene) YOU'RE COMING WITH ME",
  "the pain just does not stop",
...
function print_debug(header) {
  console.log(header);
  console.log(`
  raw_posts: ${raw_posts.length} left;
  target_post: ${target_post};
  target_post_length: ${target_post_length};
...
function type_post() {
  ...
  if (target_post.length > 0) {
    ...
    set_post_pie_percent(100 * (draft_post.length / target_post_length))
    ...
    // Can't type; post is ready to go
    return false;
  }
}

function set_post_input(value) {
  document.getElementById('post_input').value = value;
  if (draft_post.length == target_post_length) {
    document.getElementById('post_send').disabled = false;
  }
}
...
function post_send() {
  if (draft_post.length == target_post_length) {
    ...
    set_post_input("");
    // Re-disable post
    document.getElementById('post_send').disabled = true;
    // Re-set height
    const ta = document.getElementById('post_input');
...
function create_post(post_text) {
  ...
  new_post = post_template.content.cloneNode(true);

  // Get the container we want to put it in
  post_feed = document.getElementById("post_feed");
